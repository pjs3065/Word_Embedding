...
print("모델 로드 중")
f = open('/home/kang/Development/wangki/parsing/model/f_model.dat','rb')
model = pickle.load(f)
...
with graph.as_default(): # Define operations and tensors in 'graph'
    ...
    # session_conf.gpu_option.allow_growth = True # 이는 실행 과정에서 요구되는 만큼의 GPU 메모리만 할당
    ...
    with sess.as_default(): # with 사용시, Operation.run(), Tensor.run()이 이 세션에서 실행, sess.close()로 명시적으로 세션을 닫아줘야함
        ...
        print('데이터 로드 중')
        sample_list = dp.make_sample_from_input_data('/home/kang/Development/wangki/parsing/data/train.out', model)
        print('데이터 로드 끝')
        ...
        for ep in range(FLAGS.num_epochs):
            ...
            cr_test.set_file('/home/kang/Development/wangki/parsing/data/test.txt')
            ...
            if(ep % FLAGS.num_checkpoints) is 0:
                path = saver.save(sess,'/home/kang/Development/wangki/parsing/chk_point/')
