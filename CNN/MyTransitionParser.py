...
class TransitionState(object): #state 상태에서 초기화
    def __init__(self):
        self.stack = []
        self.queue = [] #아마 buffer인듯?
        self.predict = [] #예상한 predict object(dependency)가 들어감
        self.transition_sequence = [] #action 순서 저장
    ...
    def initialize_state(self, data): #data -> 1개 코퍼스
        ...
        for i in range(data.get_size() - 1, -1, -1): # range(5, 10) --> 해당 범위 5,6,7,8,9  , range(1, 10, 3) --> 1에서 10까지 3단위, 즉 1, 4, 7
            # data.get_size() -> return value = eojoel list 의 size = 11 (한 코퍼스가 11개 어절로 되어있을 경우), 따라서  10부터 -1까지 -1만큼씩 이동하면서
...
class TransitionParse(object):
    def __init__(self, model):
        ...
        self.f_model = model # object로 model을 넘겨줌 -> model = feature 종류를 초기화 및 선언
    ...
    def make_gold_corpus(self, data): # data - 1개 코퍼스
        ...
        while not self.state.is_final_state(): # queue(buffer)에 있는 어절이 없어질까지, 한 코퍼스가 끝나면 loop 빠져나감
