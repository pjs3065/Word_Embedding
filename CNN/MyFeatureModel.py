...
class FeatureModel:
    def __init__(self):
        ...
        self.pos_map = {}
        self.pos_size = 0
        self.window_size = 3 # what window size? cnn 윈도우 사이즈인듯
        self.max_length = self.window_size * 4 #what max_length? window size * 4? why 4? -> buffer, stack, sequence, hc feature? 이렇게 넷?
        ...
        self.mor_size += 1 #mor_size 올려줌

        # 똑같이 pos_map도 dummy값을 넣어줘야 하는 것 아님?
        ...
        for i in range(self.max_length): # position mark를 초기화 하는 과정인듯?
            _position_mark = [float(0)] * self.max_length # 값이 0.0 인 리스트가 max length만큼
            _position_mark[i] = float(1) # float 값 1.0, 즉 모든 값을 1.0으로 초기화
